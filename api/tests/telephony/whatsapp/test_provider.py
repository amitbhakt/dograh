...
class TestWhatsAppProvider(IsolatedAsyncioTestCase):
    ...
    async def test_initiate_call_bic_disabled(self):
        """Test initiate_call raises HTTPException when business_initiated_calls_enabled is False."""
        provider = _provider(business_initiated_calls_enabled=False)
    ...
    async def test_initiate_call_restricted_countries(self):
        """Test initiate_call raises HTTPException for restricted destination countries."""
        provider = _provider(business_initiated_calls_enabled=True)
        for num in ["+15551234567", "+201012345678", "+84912345678", "+2348012345678"]:
            with self.assertRaises(HTTPException) as ctx:
                await provider.initiate_call(
                    to_number=num,
                    webhook_url="https://example.test/webhook",
    ...
    async def test_initiate_call_permission_required(self):
        ...
        with patch.object(db_client, "get_whatsapp_call_permission_by_phone_id", AsyncMock(return_value=None)), \
            ...
            self.assertEqual(ctx.exception.status_code, 400)
            self.assertIn("Permission to call", ctx.exception.detail)
    ...
    async def test_initiate_call_success_with_granted_permission(self):
        ...
        with patch.object(db_client, "get_whatsapp_call_permission_by_phone_id", AsyncMock(return_value=mock_perm)), \
            ...
            res = await provider.initiate_call(
                to_number="+447123456789",
                webhook_url="https://example.test/webhook",
                workflow_run_id=456,
                organization_id=1,
            ...
            self.assertEqual(res.call_id, "call_wa_outbound_123")
            self.assertEqual(res.status, "initiated")
            mock_reg.assert_called_once()

    async def test_initiate_call_missing_metadata_raises_400_before_meta_call(self):
    ...
    async def test_initiate_call_cleans_up_connection_on_registration_failure(self):
        ...
        with patch.object(db_client, "get_whatsapp_call_permission_by_phone_id", AsyncMock(return_value=mock_perm)), \
            ...
            mock_client.terminate_call = AsyncMock()
            mock_get_client.return_value = mock_client
    ...
    def test_whatsapp_setup_checklist_outbound_readiness(self):
        ...
        # 1. No phone numbers and BIC disabled
        c1 = _whatsapp_setup_checklist(
        ...
        self.assertFalse(c1.ready_for_outbound)

        # 2. Has phone numbers but BIC disabled
        c2 = _whatsapp_setup_checklist(
            {"business_initiated_calls_enabled": False},
            ConfigurationSetupState(
        ...
        self.assertFalse(c2.ready_for_outbound)
        self.assertIn("Business-Initiated Calls", c2.outbound_blocked_reason)

        # 3. BIC enabled and has active phone number -> ready_for_outbound is True!
        c3 = _whatsapp_setup_checklist(
        ...
        self.assertTrue(c3.ready_for_outbound)
        self.assertIsNone(c3.outbound_blocked_reason)
...
class TestWhatsAppConfigurationDisplayAndMerge(IsolatedAsyncioTestCase):
    ...
    def test_whatsapp_permission_required_error_hierarchy(self):
        ...
        self.assertIsInstance(err, TelephonyPermissionRequiredError)
        self.assertIsInstance(err, HTTPException)
        self.assertEqual(err.status_code, 400)
        self.assertEqual(err.status, "missing")
    ...
    def test_provider_does_not_import_routes(self):
        ...
        self.assertNotIn("from api.services.telephony.providers.whatsapp.routes", src)
        self.assertIn("from api.services.telephony.providers.whatsapp.service", src)

    def test_service_pipeline_runner_registration(self):
        """Verify service layer accepts and stores pipeline runner."""
