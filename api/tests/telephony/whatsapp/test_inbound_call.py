...
class TestWhatsAppInboundCalling(IsolatedAsyncioTestCase):
    ...
    async def test_webhook_terminate_cross_worker_cleanup(self):
        ...
        with patch.object(
            ...
            self.assertEqual(mock_update.await_args.args[0], 555)
            self.assertEqual(mock_update.await_args.kwargs.get("is_completed"), True)
            self.assertEqual(mock_update.await_args.kwargs.get("state"), "completed")
